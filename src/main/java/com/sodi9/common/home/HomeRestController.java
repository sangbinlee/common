...
import com.sodi9.common.cookie.CookieController.Constants;
...
public class HomeRestController {
	...
	public void addCookie(HttpServletResponse res, String name, String value, int day) {
...
//		res.setHeader(HttpHeaders.SET_COOKIE, cookie.toString());// 마지막 쿠키만 생성됨
	}

	@GetMapping("/")
	public String index(@CookieValue("JSESSIONID") String cookie) {
		return "Greetings from Spring Boot! JSESSIONID=" + cookie;
	}
...
//			HttpSession session, 
		...
		if (member != null) {
			...
			session.setAttribute("LOGIN_USER", member.getEmail());
			...
			addCookie(res, "token2", "token2", 7 * 24 * 60 * 60);
			addCookie(res, "token3", "token3", 7 * 24 * 60 * 60);
			addCookie(res, "token4", "token4", 7 * 24 * 60 * 60);

			/**
			 * TODO 개발자 도구 Application 탭에 쿠키가 안쌓인다. TODO 개발자 도구 Application 탭에 쿠키가 안쌓인다.
			 * TODO 개발자 도구 Application 탭에 쿠키가 안쌓인다. TODO 개발자 도구 Application 탭에 쿠키가 안쌓인다.
			 * TODO 개발자 도구 Application 탭에 쿠키가 안쌓인다. TODO 개발자 도구 Application 탭에 쿠키가 안쌓인다.
